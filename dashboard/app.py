...
def render_hero(title: str, subtitle: str, badge: str = "Market Overview", df: pd.DataFrame = None, **kwargs):
    """Render humanized header with creator stamp, live market seasonality toggle, and export."""
    html_content = f"""<div class="dashboard-header">
<div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:0.75rem;">
<div>
<h1 style="color:#0F172A; font-size:1.5rem; font-weight:700; margin:0 0 0.3rem 0;">{title}</h1>
...
def render_kpi(label: str, value: str, subtext: str = "", tag: str = "", accent: str = "rose", icon: str = "", **kwargs):
...
def render_card_header(title: str, desc: str = "", **kwargs):
...
def render_insight_card(title: str, text: str, icon: str = "", **kwargs):
...
def render_sidebar(df: pd.DataFrame):
    ...
    with st.sidebar:
        st.markdown(
            """
                        <div style="padding: 0.6rem 0 1rem 0; border-bottom: 1px solid #1E293B; margin-bottom: 1rem;">
...
def page_executive_overview(df: pd.DataFrame):
    ...
    with col_ins:
        ...
        render_card_header("Key Market Takeaways", "Summary of revenue drivers and pricing patterns")
        ...
        render_insight_card("Top Revenue Locality", f"<b>{top_neigh}</b> dominates gross revenue with an estimated <b>₹{top_rev_val:,.0f}</b> in annual market capture.")
        ...
        render_insight_card("Superhost Premium", f"Superhosts generate an estimated <b>{abs(sh_rev_delta)*100:.1f}% {'higher' if sh_rev_delta>=0 else 'lower'}</b> annual revenue compared to non-superhost peers.")
        
        underpriced_count = (df["pricing_opportunity"] == "Underpriced").sum() if "pricing_opportunity" in df.columns else 0
        render_insight_card("Pricing Adjustment Opportunities", f"Identified <b>{underpriced_count:,} underpriced listings</b> that could adjust nightly rates upward by ₹1,000–₹2,500 without sacrificing occupancy.")
        st.markdown("</div>", unsafe_allow_html=True)

    with col_rec:
        st.markdown("<div class='content-box'>", unsafe_allow_html=True)
        render_card_header("Host Pricing & Operating Notes", "Observations from historical booking and review data")
    ...
    tab_map_2d, tab_map_3d, tab_street_view, tab_geo_chart = st.tabs([
        "OpenStreetMap View",
        "3D Density Map",
        "Street View & Localities",
        "Locality Price Rankings"
    ])
